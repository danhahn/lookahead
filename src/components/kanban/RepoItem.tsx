...
type Props = {
  ...
  leftArrow?: boolean;
  rightArrow?: boolean;
  progress?: (id: string, column: Columns) => void;
  degrees?: (id: string, column: Columns) => void;
  columnName: Columns;
};
...
const RepoItem: React.FC<Props> = ({
  ...
  return (
    <Li>
      <button
        type="button"
        onClick={() => degrees?.(repo.id, columnName)}
      ...
      <button
        type="button"
        onClick={() => progress?.(repo.id, columnName)}
